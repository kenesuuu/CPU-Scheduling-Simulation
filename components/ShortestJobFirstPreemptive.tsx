...
const ShortestJobFirstPreemptive: React.FC = () => {
  ...
  const runShortestJobFirstPreemptive = () => {
    ...
    while (true) {
      ...
      let shortestJobBurst = Number.MAX_VALUE;
      for (let i = 0; i < sortedProcesses.length; i++) {
        if (sortedProcesses[i].arrivalTime <= currentTime && remainingBurstTimes[i] > 0 && sortedProcesses[i].burstTime < shortestJobBurst) {
          shortestJobIndex = i;
          shortestJobBurst = sortedProcesses[i].burstTime;
        }
      }
      if (shortestJobIndex === -1) {
        ...
        if (remainingBurstTimes[shortestJobIndex] === 0) {
          sortedProcesses[shortestJobIndex].completionTime = currentTime;
          sortedProcesses[shortestJobIndex].turnaroundTime = sortedProcesses[shortestJobIndex].completionTime - sortedProcesses[shortestJobIndex].arrivalTime;
          sortedProcesses[shortestJobIndex].waitingTime = sortedProcesses[shortestJobIndex].turnaroundTime - sortedProcesses[shortestJobIndex].burstTime;
        }
      }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <button onClick={runShortestJobFirstPreemptive} className="bg-black text-white py-2 px-4 rounded-md hover:bg-gray-700 mt-4 mr-2">
